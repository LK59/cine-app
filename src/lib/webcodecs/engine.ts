...
// Text subtitle codecs the engine can render itself. ASS and SSA are here for their words only:
// the styling, the positioning and any embedded fonts are dropped, which for ordinary dialogue
// is invisible and for a positioned sign is a line at the bottom like any other. Offering them
...
export class PlaybackEngine {
  ...
  private async startSoftwareAudio(
    ...
    try {
      software = await SoftwareAudioTrack.open(this.source, track.number, track.codecId, this.file ?? undefined);
    } catch (error) {
