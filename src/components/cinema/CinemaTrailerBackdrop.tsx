...
// A locally-downloaded file (see trailerDownload.ts) starts almost instantly and has none of
// YouTube's iframe chrome to wait out — so the dwell can be much shorter than the YouTube
// fallback path needs.
const DWELL_MS_LOCAL = 2000;
const DWELL_MS_YOUTUBE = 3000;
...
// the frame, it's now cropped away rather than depending purely on timing. Only applies to the
// YouTube fallback path — a local <video>'s own onPlaying is trustworthy immediately.
...
// should already be darkest. Applies identically to either video source below.
const VIGNETTE = "radial-gradient(ellipse 80% 75% at 64% 36%, transparent 35%, transparent 52%, rgba(2,6,23,0.6) 78%, rgb(2,6,23) 100%)";

// Netflix's own browse-hero behavior: once focus has rested on a title for a dwell period, its
// trailer takes over the persistent backdrop banner itself — same role, same full-screen
// footprint CinemaClient's still-image backdrop already has (own vignette here instead of that
// image's own vertical mask, which was tuned for a still photo fading into the rows pane below,
// not a video meant to stay bold across most of the frame) — rather than a separate box floating
// on top of the UI. Rendered as a sibling layered directly over that still image (later in DOM
// order, no z-index needed — see CinemaClient's own note on that convention); the image never
// unmounts underneath it, so it's always there as the fallback until (and unless) this actually
// becomes visible. Shared by CinemaHero's and CinemaSeriesHero's tabs via CinemaClient, which
// owns the debounced hero item this is keyed on — the logic here has nothing movie/series-specific
// in it, just a trailer key/local file url and something to key the dwell timer on.
//
// Prefers a locally-downloaded file (plain <video>, instant start, no iframe chrome) over the
// live YouTube embed (YT IFrame Player API, with the settle/crop workarounds above) — see
// trailerDownload.ts for how/why those files get downloaded.
export function CinemaTrailerBackdrop({
  ...
  const hasSource = !!trailerKey || !!localTrailerUrl;
  ...
  // `trailerKey`/`localTrailerUrl` themselves settle: arrowing across a row must never leave a
  // stale dwelled/playing/unmuted state, or a previous title's player instance, bleeding into
  // the next one.
  ...
  useEffect(() => {
    if (!hasSource) return;
    const timer = setTimeout(() => setDwelled(true), localTrailerUrl ? DWELL_MS_LOCAL : DWELL_MS_YOUTUBE);
    return () => clearTimeout(timer);
  }, [itemKey, hasSource, localTrailerUrl]);

  // Creates a real YT.Player once dwelled — not a raw <iframe src=...>, and only for the
  // fallback (no local file) path. Two things this buys over a plain embed: an actual
  // onStateChange event to key the reveal off of (see SETTLE_MS's own note — a fixed timer had
  // to guess long enough to probably outlast YouTube's own startup overlay, and still sometimes
  // didn't), and mute()/unMute() calls that don't require reloading the player (a raw iframe
  // would need its src rewritten, restarting playback from 0).
  useEffect(() => {
    if (!dwelled || !trailerKey || localTrailerUrl || !mountRef.current) return;
    let cancelled = false;
  ...
  }, [dwelled, trailerKey, localTrailerUrl, itemKey]);
  ...
  if (!hasSource || !dwelled) return null;
